...
//== POSICAO X == LAYER NULL SCROLL A ========

posTela = thisComp.layer("NULL_CONTROLES").effect("posicaoNaTela")("Menu");
larguraMax = Math.max(thisComp.layer("FUNCOES A").sourceRectAtTime(0).width, thisComp.layer("NOMES A").sourceRectAtTime(0).width);
...
// TESTE QUE USA % INICIAL -- ESTÁ ERRADO  ********************************************************
TESTE NULL A COMEÇA ADIANTADO

h = thisComp.height
ciclo = thisComp.layer("NULL_VARIAVEIS").effect("ciclo")("Slider")  //em segundos

a2 = thisComp.layer("NULL_CONTROLES").effect("ajuste_animacao_fim(%)")("Slider");
...
if(time <= ciclo)
{
	...
	p0 = h*a2/100
	t22 = (p0 + h)/vel + t1
	linear(time, t1, t22, p0, p1)
}
else
{
	linear(time, t1, t2, p0, p1)	
}
...
//== POSICAO X === LAYER NULL SCROLL B ========
thisComp.layer("NULL_SCROLL_A").transform.position[0]


//== POSICAO Y LAYER NULL SCROLL B ========
...
//TOTAL_LINHAS
//A redução do percentual do final da animação corresponde à redução do mesmo percentual de linhas.
//Isto leva a uma redução do número de telas e consequentemente a uma redução da velocidade, pois o ciclo aumentará.
a2 = thisComp.layer("NULL_CONTROLES").effect("ajuste_animacao_fim(%)")("Slider");

linhas = thisComp.layer("PREENCHER").text.sourceText.split("\r").length
Math.round(linhas*a2/100);
...
//MEU CÁLCULO É DIFERENTE DO ORIGINAL ***************
//O ciclo é aumentado em função de a2
if(a2>0)
{
	ciclo = (100/a2)*ciclo
}
